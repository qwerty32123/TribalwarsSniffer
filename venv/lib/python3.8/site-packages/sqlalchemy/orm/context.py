...
class ORMCompileState(CompileState):
    ...
    def orm_setup_cursor_result(
        ...
        # cover edge case where ORM config used in legacy select
    ...
    def _lead_mapper_entities(self):
        """return all _MapperEntity objects in the lead config collection.

        Does **not** include config that have been replaced by
...
class ORMFromStatementCompileState(ORMCompileState):
    ...
    def create_for_statement(cls, statement_container, compiler, **kw):
        ...
        if isinstance(
            ...
            # setup for all config. Currently, this is not useful
...
class ORMSelectCompileState(ORMCompileState, SelectState):
    ...
    def _setup_for_generate(self):
        ...
        if query._correlate:
            # ORM mapped config that are mapped to joins can be passed
    ...
    def _create_entities_collection(cls, query, legacy):
        ...
        # config will also set up polymorphic adapters for mappers
    ...
    def _join_determine_implicit_left_side(
        ...
        if self.from_clauses:
            ...
            raise sa_exc.InvalidRequestError(
                "No config to join from; please use "
    ...
    def _join_place_explicit_left_side(self, entities_collection, left):
        ...
        # with existing mapper config, in which case we want to apply the
        ...
        if (
            ...
            for idx, ent in enumerate(entities_collection):
                # TODO: should we be checking for multiple mapper config
                # matching?
