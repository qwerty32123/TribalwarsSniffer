...
def instances(cursor, context):
    ...
    # query has ORM http_entities, so legacy deduping and scalars should be called
    # on the result.
...
def _setup_entity_query(
    ...
    **kw
    ...
    # for the lead http_entities in the path, e.g. not eager loads, and
    ...
    if not refresh_state and _polymorphic_from is not None:
        ...
        if key in context.attributes and context.attributes[key].strategy == (
            ...
            selectin_load_via = mapper._should_selectin_load(
                context.attributes[key].local_opts["http_entities"],
                _polymorphic_from,
            )
        else:
            selectin_load_via = mapper._should_selectin_load(
