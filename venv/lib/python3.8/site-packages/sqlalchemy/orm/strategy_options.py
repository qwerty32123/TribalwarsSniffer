...
class Load(Generative, LoaderOption):
    ...
    def process_compile_state_replaced_entities(
        ...
        # against what the lead config were, as well as to accommodate
        # for the config having been replaced with equivalents
        self._process(
            compile_state,
            mapper_entities,
        ...
        for ent in entities:
            ...
            if raiseerr:
                if not list(entities):
                    raise sa_exc.ArgumentError(
                        "Query has only expression-based config, "
                    ...
                    raise sa_exc.ArgumentError(
                        ...
                        "root config in this query, e.g. %s. Please "
                        "specify the full path "
                        "from one of the root config to the target "
                        "attribute. "
            ...
            if len(list(entities)) != 1:
                if raiseerr:
                    raise sa_exc.ArgumentError(
                        ...
                        "loader option to multiple config %s. Specify "
...
def load_only(loadopt, *attrs):
    ...
    For a :class:`_query.Query` that has multiple config,
