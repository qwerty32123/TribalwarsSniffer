...
class Load(Generative, LoaderOption):
    ...
    that has multiple http_entities::
    ...
    def process_compile_state_replaced_entities(
        ...
        # against what the lead http_entities were, as well as to accommodate
        # for the http_entities having been replaced with equivalents
        self._process(
            compile_state,
            mapper_entities,
        ...
        for ent in entities:
            ...
            if raiseerr:
                if not list(entities):
                    raise sa_exc.ArgumentError(
                        "Query has only expression-based http_entities, "
                    ...
                    raise sa_exc.ArgumentError(
                        ...
                        "root http_entities in this query, e.g. %s. Please "
                        "specify the full path "
                        "from one of the root http_entities to the target "
                        "attribute. "
            ...
            if len(list(entities)) != 1:
                if raiseerr:
                    raise sa_exc.ArgumentError(
                        ...
                        "loader option to multiple http_entities %s. Specify "
...
def load_only(loadopt, *attrs):
    ...
    For a :class:`_query.Query` that has multiple http_entities,
